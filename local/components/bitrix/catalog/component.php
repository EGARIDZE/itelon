...
if ($arParams["SEF_MODE"] === "Y")
{
	...
	if($b404 && CModule::IncludeModule('iblock'))
	{
		...
		if ($folder404 != $APPLICATION->GetCurPage(true))
		{
			...
			if($arrUrl[1] == 'category'){
				//Get element from iblock 18 (Sets)				
				\Bitrix\Main\Loader::includeModule('iblock');

				$dbItems = \Bitrix\Iblock\ElementTable::getList(array(
					'order' => [],
					'select' => array('ID', 'NAME', 'IBLOCK_ID', 'SORT', 'TAGS'),
					'filter' => array('IBLOCK_ID' => 18, 'ACTIVE' => 'Y', 'CODE' => $arVariables["SECTION_CODE_PATH"]),
					'limit' => 1,
				));				

				if( !empty($dbItems->fetch()) ){
					$arParams["SET_STATUS_404"] = 'N';
					$arParams["SHOW_404"] = 'N';
				}
			}			
			//End custom /category/ pages
