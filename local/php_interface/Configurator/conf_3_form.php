...
//////////////////////////
Asset::getInstance()->addJs('/local/php_interface/Configurator/conf.js');
$APPLICATION->SetAdditionalCSS('/local/php_interface/Configurator/conf.css');
include_once 'EConfigIntConst.php';
include_once 'EConfigStrConst.php';
include_once __DIR__. "/../../Test/console_log.php";
const POT = EConfigStrConst::POT->value;
const PGT = EConfigStrConst::PGT->value;

function _ShowStringPropertyField_new(
        $name
        , $property_fields
        , $values
        , $bInitDef = false
...
)
{
    ...
    foreach($values as $key=>$val)
    {
        ...
        if (is_array($val) && array_key_exists("VALUE", $val))
        {
            ...
            $val = $val["VALUE"];
            $cntClass = $val_description ? ' colored' : '';
        }
        echo "<div class='flex-row'>";
            printSelectWithFilter($arOptions, $val);
            echo "<input class='input-sku' name='$name" . "[$key]$strAddDesc' value='"
            .htmlspecialcharsbx($val)."' size='30' type='text'>";

            if ($property_fields["WITH_DESCRIPTION"] == "Y")
                echo '<input class="input-cnt'.$cntClass.'" name="'.$name.'['.$key.'][DESCRIPTION]" value="'.htmlspecialcharsbx($val_description)
                .'" size="10" type="text" id="'.$name.'['.$key.'][DESCRIPTION]" placeholder="Кол-во">';
        echo "</div>";
    ...
    if (!$bVarsFromForm || $show)
    {
        for ($i = 0; $i < $cnt; $i++)
        {
            if ($i == 0 && $bInitDef)
                ...
                $val = "";
            echo "<div class='flex-row'>";
            printSelectWithFilter($arOptions);
            echo "<input class='input-sku' name='$name"."[n" . ($start + $i) . "]$strAddDesc' value='"
            . htmlspecialcharsbx($val) . "' size='30' type='text' placeholder='Артикул'>";

            if ($property_fields["WITH_DESCRIPTION"] == "Y")
                echo '<input class="input-cnt" name="'.$name.'[n'.($start + $i).'][DESCRIPTION]" value="" size="10" type="text"
                    placeholder="Кол-во">';
    ...
    if ($property_fields["MULTIPLE"] == "Y")
    {
        echo "<input type='button' class='add-more-btn' value='Добавить'>";
    }

}
function printSelectWithFilter($arOptions, $v = false){
    ?>
    <select class="option-select">
        <option value="0" disabled <?=!$v?'selected':''?>>Выбрать опцию</option>
        <?
        foreach ($arOptions as $optSku => $optName)
            echo "<option value='$optSku' ".($v === $optSku ? 'selected' : '').">$optName</option>";
        ?>
    </select>
    <input class="option-filter" type="text" size="30" placeholder="Фильтр опций">
    <?
}
// Кнопка "Настроить"
$aMenu = array();
...
include($_SERVER['DOCUMENT_ROOT'].'/test/console_log.php');
...
if($arTranslit["TRANSLITERATION"] == "Y")
{
    ...
    <tr id="tr_NAME">
        ...
        <td style="white-space: nowrap;">
            <input type="text" size="100" name="NAME" id="NAME" maxlength="255" value="<?echo $str_NAME?>"><img id="name_link" title="<?echo GetMessage("IBEL_E_LINK_TIP")?>" class="linked" src="/bitrix/themes/.default/icons/iblock/<?if($bLinked) echo 'link.gif'; else echo 'unlink.gif';?>" onclick="set_linked()" />
        </td>
    </tr>
    <?
...
if(!empty($PROP)):
    ...
    // region options harvest
    // Массив опций для модели
    //clog($PROP);
    $model = reset($PROP['MODEL']['VALUE']) ?? false;
	$modelGroup = [];
	$skuFilter = reset($PROP['FILTER_BY_SKU']['VALUE']) ?? false;
    $arOpTypesList = [];
	$arOptList = [];
	//clog(CIBlockPropertyEnum::GetByID($optBindId));
	...
	if ($model) {
		...
		while ($ob = $res->Fetch()) {
            $modelGroup = $ob['PROPERTY_GROUP_VALUE'];
		    $arOpTypesList[$ob[POT.'_CODE']] = $ob[POT.'_NAME'];
		    if ( empty($arGlobalSubTypes) && !empty($ob[PGT.'_VALUE']) ) $arGlobalSubTypes = $ob[PGT.'_VALUE'];
		}

	    $arPrivateSubTypes = array_values($PROP['SUBTYPE_BIND']['VALUE']); // Привязка к подтипу опций 1136
        $arSubTypes = array_unique(array_merge($arPrivateSubTypes, $arGlobalSubTypes));

		$arFilter = [
			"IBLOCK_ID" => EConfigIntConst::OPT_IBLOCK_ID->value
			,'PROPERTY_TYPE.CODE' => array_keys($arOpTypesList)
            ,'ACTIVE' => 'Y'
			,[
				"LOGIC" => "OR"
				,["PROPERTY_MODELS" => $model]
                ,[
					"PROPERTY_MODELS_GROUP" => $modelGroup
					,"!PROPERTY_EXC_MODELS" => $model
				]
				,["PROPERTY_MODELS" => $modelGroup ,"!PROPERTY_EXC_MODELS" => $model]
			]
			,[
				"LOGIC" => "OR"
				, ["PROPERTY_SUBTYPE" => false]
				, ["PROPERTY_SUBTYPE" => $arSubTypes]
			]
			,[
				"LOGIC" => "OR"
				, ["PROPERTY_SKU_POOL" => false]
				, ["PROPERTY_SKU_POOL" => $str_CODE]
			]
		];
...
//            , 'PROPERTY_SHORT_NAME'
        ...
        while ($ob = $res->Fetch()) {
            $name = $ob['NAME'];
            $arOptList[$ob['PROPERTY_TYPE_CODE']][$ob['CODE']] = $name;
        }
    }
//foreach($arOptList as $k => $v)$cnt+=count($v);clog($cnt);
    $arMissedOptions = array_diff_key( $arOpTypesList, $arOptList);
	if ($arMissedOptions) {
		$html = '<div class="missed-alert">Для следующих типов опции не определены:';
		foreach ($arMissedOptions as $value) $html .= "<p>$value</p>";
		$html .= '</div>';
		echo $html;
	}
//    echo "<p>TEST</p>";
// end region
    foreach($PROP as $prop_fields):
        $prop_code = $prop_fields['CODE'];
        $propType = '';
        if (str_starts_with($prop_code, 'INS'))
            $propType = explode('_', $prop_code)[1];
        ...
        <tr id="tr_PROPERTY_<?echo $prop_fields["ID"];?>"<?if ($prop_fields["PROPERTY_TYPE"]=="F"):?> class="adm-detail-file-row"<?endif?>>
            <?
            if ($propType) {
                if (array_key_exists($propType, $arOptList)) {
                ?>
                    <td colspan="2">
                        <div class="installed-wrap<? if($prop_fields['VALUE']) echo ' filled' ?>">
                            <p class="wrap-label"><? echo $tabControl->GetCustomLabelHTML();?></p>
                            <div class="flex-column">
                                <?
                                _ShowStringPropertyField_new(
                                    'PROP['.$prop_fields["ID"].']'
                                    , $prop_fields
                                    , $prop_fields["VALUE"]
                ...
                if ($prop_fields["ID"] != 1136) {   //привязка к подтипу
